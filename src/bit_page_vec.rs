    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> Result<(), fmt::Error> {
        match self.kind {
            ...
            BitPageVecKind::SparseWithZeroesHole => write!(
                f,
                "BitPageVec::SparseWithZeroes(len={}, last_bit_index={:?}, active_bits={}, start_page={:?}, end_page={:?}",
                self.size(),
                self.last_bit_index,
            ...
            BitPageVecKind::SparseWithOnesHole => write!(
                f,
                "BitPageVec::SparseWithOnesHole(len={}, last_bit_index={:?}, active_bits={}, start_page={:?}, end_page={:?}",
                self.size(),
                self.last_bit_index,
                BitPageVec::count_ones(self.pages.as_ref()),
                BitPageVec::start_page(self.pages.as_ref()),
